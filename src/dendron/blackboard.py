...
class Blackboard:
    ...
    def set_entry(self, key : Any, description : Optional[str] = None) -> None:
        """
    ...
    def __setitem__(self, key : Any, value : Any) -> None:
        ...
        in the blackboard, creates a new entry with an auto-generated description.
        ...
        if key not in self.entry_mapping.keys():
            new_entry = BlackboardEntryMetadata(key, "autogen")
    ...
    def __str__(self) -> str:
        ...
        The value column will expand to fit content while keeping total width under 120 chars.

        Returns:
            `str`: A string that prints as a table of keys and values.
        """
        # Calculate max value length while keeping total line under 120 chars
        # Format is: "key | description | value |"
        # So we need: key_width + desc_width + value_width + 8 <= 120
        max_value_width = 120 - (self.print_len * 2 + 8)  # 8 accounts for separators
        
        # Find the longest value string (up to max_value_width)
        value_width = 0
        for value in self.value_mapping.values():
            value_str = str(value)
            value_width = min(max(value_width, len(value_str)), max_value_width)

        # Use at least self.print_len for value column
        value_width = max(value_width, self.print_len)

        key_header_field = f"{'Key':{self.print_len}.{self.print_len}}"
        desc_header_field = f"{'Description':{self.print_len}.{self.print_len}}"
        value_header_field = f"{'Value':{value_width}.{value_width}}"
        bb_str = f"{key_header_field} | {desc_header_field} | {value_header_field} |\n"
        bb_str += f"{'=' * (self.print_len * 2 + value_width + 8)}\n"
        
        for key, value in self.value_mapping.items():
            entry = self.entry_mapping[key]
            value_string = str(value)
            if len(value_string) > value_width:
                value_string = value_string[:value_width-3] + "..."
            value_field = f"{value_string:{value_width}.{value_width}}"
